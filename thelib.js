...
class main {
    ...
    tileMapEditor(tiles = [],images = [],url,tileSize){
        ...
        image.addEventListener("touchstart", (ev) => {
            ...
            slect.style.top = this.mousePos.y * tileSize + "px"
            slect.style.left = this.mousePos.x * tileSize + "px"
            slectPos.equal(this.mousePos.x, this.mousePos.y)
        })
        document.body.appendChild(slect)
        
        this.canvas.style.left.substring(this.canvas.style.left.length - 1,2)
        const canvasx = JSON.parse(this.canvas.style.left)
        this.canvas.addEventListener("touchstart", ev => {
            this.mousePos.equal(canvasx > 0? ev.touches[0].clientX - this.canvas.width / 2 : ev.touches[0].clientX, ev.touches[0].clientY)
            tiles.forEach(tile => {
                if (collide.pointInBox(this.mousePos, tile)) {
                    images.push(new drawImg(this.canvas, new vector(tile.pos.x, tile.pos.y), tile.size, 'test make tilemap in js canvas.png', new vector(tileSize, tileSize)))
                    images[images.length - 1].posOfTile = new vector(slectPos.x, slectPos.y)
                }
            })
        })
    }
